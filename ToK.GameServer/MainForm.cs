        ...
        private async void ProcessClient_Channel1(TcpClient client)
        {
            ...
            using (NetworkStream stream = client.GetStream())
            {
                ...
                try
                {
                    ...
                    while (gameController.PlayerState[player] != EPlayerState.CLOSED)
                    {
                        ...
                        else // Possible valid game packet chunk.
                        {
                            unsafe
                            {
                                ...
                                fixed (byte* PinnedPacketChunk = &packet.RawBuffer[packet.Offset])
                                {
                                    ...
                                    while (packet.Offset < readCount && gameController.PlayerState[player] != EPlayerState.CLOSED)
                                    {
                                        ...
                                        ERequestResult packErr = PlayerRequestController.TryProcessPacket(gameController, player);

                                        // Treat the processing packet return.
                                        switch (packErr)
                                        {
                                            case ERequestResult.PACKET_NOT_HANDLED:
                                            ...
                                            case ERequestResult.CHECKSUM_FAIL:
